        ...
        <main class="agent-help-main">
            ...
            <section class="agent-help-section" id="earnings"><div class="agent-help-section-head"><span>04</span><div><h2>Earnings</h2><p>How qualifying order commission is recorded.</p></div></div>
                <div class="agent-help-questions">
                    ...
                    <details><summary>How is my commission calculated?</summary><p>Your current share is {{ number_format($currentAgentRate, 2) }}% of Pahatud’s commission on a qualifying order, rather than the full order value. For example, if an eligible ₱1,000 order uses a {{ number_format(config('agent.pahatud_commission_percentage'), 2) }}% Pahatud rate, your share would be ₱{{ number_format(1000 * config('agent.pahatud_commission_percentage') / 100 * $currentAgentRate / 100, 2) }}. The rate saved with each order controls its actual calculation.</p></details>
                    <details><summary>How do commission tiers work?</summary><p>You receive 15% of Pahatud’s commission while you have fewer than 35 approved restaurants, 20% with 35–49, and 30% with 50 or more. Only currently approved restaurants count. When you reach a new tier, its rate applies to future qualifying orders from every restaurant assigned to you; existing commission entries keep their original rate.</p></details>
