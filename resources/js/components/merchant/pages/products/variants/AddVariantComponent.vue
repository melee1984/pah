    ...
    <div class="modal-dialog modal-xl">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h4><span class="glyphicon glyphicon-lock"></span> Manage Variant</h4>
              ...
              <div class="row">
                ...
                <div class="col-6 text-right">
                  <a
                    ...
                    v-if="actionVariant == 'view'"
                    v-on:click="manageVariant('manage')"
                    class="btn btn-pahatud btn-sm mb-2 pull-right"
                  >
                  ...
                  >
                    <td v-if="actionVariant != 'view'" align="center" width="5%">
                      <a
                        href="javascript:void(0)"
                        class="text-danger"
                        v-on:click="editVariantDetails(detail)"
                      >
                    ...
                    <td v-if="actionVariant != 'view'" align="center" width="5%">
                      <a
                        href="javascript:void(0)"
                        class="text-danger"
                        v-on:click="deleteVariantDetails(detail)"
                      >
            ...
            <div class="col-4">
              ...
              <div class="card card-pahatud">
                ...
                <div class="card-body">
                  ...
                  <form
                    method="post"
                    v-on:submit.prevent="submitRecord"
                    v-if="!actionSuccess"
                  >
                    <div class="form-group">
                      <div class="custom-control custom-switch">
                        <input
                          type="checkbox"
                          class="custom-control-input"
                          :id="'details_' + field.active"
                          v-model="field.active"
                        />
                        <label
                          class="custom-control-label"
                          :for="'details_' + field.active"
                        >
                          Selected by Default</label
                        >
                      </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                      <input
                        ...
                        step="any"
                        name="variant_price"
                        placeholder="Enter Add up Price"
                        required
                        v-model="field.price"
                        @change="updatePriceComm()"
                      />
                    </div>
                    <div class="form-group">
                      <input
                        type="text"
                        class="form-control"
                        id="variant_price_comm"
                        name="variant_price"
                        placeholder=""
                        disabled=""
                        v-model="field.price_comm"
                      />
                    </div>
                    <button type="submit" class="btn btn-pahatud btn-block">
                      <span v-if="!isSubmit">Submit</span>
                      <span v-if="isSubmit">Please wait...</span>
                    </button>
                  </form>
                  <!-- /Form -->
                </div>
...
export default {
  ...
  methods: {
    updatePriceComm: function (event) {
      this.field.price_comm = this.field.price * 0.15;
    },
    ...
    reloadVariantDetails: function (product_header_id) {
      ...
      var self = this;
      axios
        .get("/api/merchant/product/variant/" + product_header_id + "/details")
    ...
    submitRecord: function () {
      ...
      if (this.validateForm()) {
        if (this.actionVariant != "edit") {
          axios
            .post(
              "/api/merchant/product/variant/" +
                self.product_header_id +
                "/detail/submit",
              {
    ...
    deleteVariantDetails: function (detail) {
      ...
      if (r == true) {
        axios
          .delete("/api/merchant/product/variant/detail/" + detail.id + "/delete")
          .then(function (response) {
            if (response.data.status) {
              toastr.success(response.data.message);
    ...
    openModalView() {
      if (!this.modalInstance) {
        const modalEl = document.getElementById("modalAddVariantDetails");
