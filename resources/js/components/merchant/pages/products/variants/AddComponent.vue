          ...
          <form method="post" v-on:submit.prevent="submitRecord" v-if="!actionSuccess">
            <div class="form-group">
              <div class="custom-control custom-switch">
                <input
                  ...
                  :id="'active_' + field.active"
                  v-model="field.active"
                />
                <label class="custom-control-label" :for="'active_' + field.active">
                  Active</label
                >
              </div>
            </div>
            <div class="form-group">
              <div class="custom-control custom-switch">
                <input
                  type="checkbox"
                  class="custom-control-input"
                  :id="'required_' + field.required"
                  v-model="field.required"
                />
                <label class="custom-control-label" :for="'required_' + field.required">
                  Required</label
                >
              </div>
            </div>
            <div class="form-group">
              <div class="custom-control custom-switch">
                <input
                  type="checkbox"
                  class="custom-control-input"
                  :id="'required_' + field.is_multiple"
                  v-model="field.is_multiple"
                />
                <label
                  class="custom-control-label"
                  :for="'required_' + field.is_multiple"
                >
            ...
            <div class="form-group">
              <input
                ...
                id="sorting"
                name="sorting"
                placeholder="Enter sorting eg. 1"
                required
                v-model="field.sorting"
              />
            </div>
...
export default {
  ...
  props: ["productId"],
  mounted() {
    console.log("Component mounted variant add component.");
    this.initModal();
  },
  methods: {
    submitRecord: function () {
      ...
      if (this.validateForm()) {
        axios
          .post("/api/merchant/product/variant/submit", {
    ...
    openModalView() {
      if (!this.modalInstance) {
        const modalEl = document.getElementById("modalAddVariant");
